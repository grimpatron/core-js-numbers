...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.abs(Math.atan2(determinant, point));
}
...
function getFibonacciNumber(index) {
  ...
  const result = getFibonacciNumber(index - 1) + getFibonacciNumber(index - 2);
  return result;
}
...
function getSumOfDigits(num) {
  ...
  return newArr.reduce((sum, item) => sum + Number(item), 0);
}
...
function getRandomInteger(min, max) {
  const result = Math.round((max - min) * Math.random() + min);
  return result;
}
